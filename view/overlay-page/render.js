$(window).on('load', function() {
    ...
    $(".cosplayer-temp-img").each(function () {
        ...
        $(".cosplayer-latest").append(`
            <a href="#" class="cosplayer-link w-inline-block openPhotoOverlay" style="background-image:url('` + pictureResize(500, $(this).attr("src")) + `')" id="` + $(this).attr("src").split('/').pop().split('_', 1) + `" data-src="` + $(this).attr("src") + `" data-srcset="` + $(this).attr("srcset") + `">
    ...
    $(".openPhotoOverlay").click(function () {
        ...
        $(".photo-column").children("img").attr("src", $(this).attr("data-src")).attr("srcset", $(this).attr("data-srcset")).attr("loading","lazy");
